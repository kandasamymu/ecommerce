    ...
    if current_user
      ...
      if get_cart_orders_current_user && get_cart_orders_current_user.first
        get_cart_orders_current_user.first.order_products.each do |order_product|
          ...
          @get_cart_order_products_with_calculated_price["total_price"] = @get_cart_order_products_with_calculated_price["total_price"] + (order_product.product_price * order_product.product_quantity)
        end
      end
    end
  end
